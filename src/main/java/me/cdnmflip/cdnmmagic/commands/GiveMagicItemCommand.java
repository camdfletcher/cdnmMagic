...
public class GiveMagicItemCommand implements CommandExecutor {
  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    ...
    try {
      ...
      if (amount > 64) {
        ChatUtil.error(player,
            "Too many items specified! &7(You cannot give more than 64 items at once)");
        return true;
      }
    } catch (NumberFormatException e) {
      ChatUtil.error(player,
          "Invalid amount specified! Make sure it's a valid number. (e.g. 1, 2, 10, 64)");
      return true;
    }

    ChatUtil.msg(player,
        "&aYou have been given &ex" + amount + " " + item.get().getDisplayName() + "&a!");
    ...
    if (player.getInventory().firstEmpty() == -1) {
      ...
      player.getWorld()
          .dropItem(player.getLocation(), item.get().generateItemStack(player, amount));
      ChatUtil.msg(player,
          " &7&o(There was not enough space in your inventory so the items were placed on the ground!)");
    } else {
