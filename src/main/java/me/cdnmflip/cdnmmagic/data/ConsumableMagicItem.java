...
public abstract class ConsumableMagicItem extends MagicItem {
  ...
  public ConsumableMagicItem(MagicItemType type, String identifier, String displayName,
      long cooldown, String... description) {
    super(type, identifier, displayName, cooldown, description);
  }

  /**
   * Determines how the {@link ItemStack} should be handled when consumed/activated e.g. If the item
   * is an ITEMSTACK item type then remove one of the items from the potential stack
   *
  ...
  /**
   * Automatically handle the generic consumption of a spell e.g. Remove one item from a stack or
